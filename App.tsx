...
const App: React.FC = () => {
  ...
  const [language, setLanguage] = useState<Language>(() => (localStorage.getItem('smart_lang') as any) || 'pt');
  ...
  if (!session.isAuthenticated) return <Login onLogin={() => {}} onRealLogin={async (email, password, keep) => {
    try {
      // FIX: Cast res to any to avoid "Property 'user' does not exist on type 'unknown'" and "Property 'token' does not exist on type 'unknown'"
