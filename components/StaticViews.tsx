...
const SectorialInfo: React.FC<{t: any}> = ({ t }) => {
  const handleDownload = async () => {
    ...
    } catch (e) { alert("Report not available yet."); }
  };
...
const AboutUs: React.FC<{t: any}> = ({ t }) => {
  ...
  useEffect(() => {
    fetchAboutUs()
      .then(res => {
        const text = Array.isArray(res) && res.length > 0 ? (res[0]?.about || res[0]?.text || '') : '';
        setContent(text);
      })
      .catch(() => setContent(''))
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12 space-y-8 animate-in fade-in">
      ...
      <div className="glass p-8 md:p-12 rounded-[40px] shadow-sm relative overflow-hidden min-h-[200px]">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 text-center space-y-4">
            ...
            <p className="text-slate-400 font-black uppercase tracking-widest italic">Information temporarily unavailable.</p>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="max-w-4xl mx-auto px-4 py-12 space-y-8 animate-in fade-in">
      ...
      <div className="space-y-4">
        {faqs.length > 0 ? faqs.map((faq, idx) => (
          <div key={idx} className="glass rounded-[28px] overflow-hidden border border-black/5">
            <button 
