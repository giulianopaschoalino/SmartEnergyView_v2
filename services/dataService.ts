...
const normalizeTelemetry = (record: any): EnergyRecord => {
  const rawDate = record.dia_num || record.dia_formatado || record.day_formatted || record.timestamp;
  const timestamp = (rawDate && !isNaN(Date.parse(rawDate))) ? rawDate : new Date().toISOString();
  ...
  login: async (credentials: any) => {
    ...
    return {
      ...
      user: {
        ...
        scdeCode: response.user.scde_code || response.user.codigo_scde,
...
export const fetchConsumptionLogs = async (days: number = 30, scdeCode?: string): Promise<EnergyRecord[]> => {
  try {
    ...
    const filters: any[] = [{
      type: 'between', field: 'dia_num', 
      value: [startDate.toISOString().split('T')[0], now.toISOString().split('T')[0]]
    }];
    if (scdeCode) filters.push({ type: '=', field: 'med_5min.ponto', value: scdeCode });
    const response = await ApiClient.post<any[]>('/telemetry/discretization', { type: '1_dia', filters });
    return (!response || response.length === 0) ? generateMockData(days) : response.map(normalizeTelemetry);
  } catch (error) { return generateMockData(days); }
};

export const fetchEconomyData = async (type: 'grossAnnual' | 'grossMonthly' | 'estimates' | 'MWh', filters: any[] = []) => {
  try {
    return await ApiClient.post<any[]>(`/economy/${type}`, { filters });
  } catch (e) { return []; }
};

export const fetchPLDData = async (type: 'overview' | 'list' | 'daily' | 'schedule', filters: any[] = []) => {
  try {
    return await ApiClient.post<any>(`/pld/${type}`, { filters });
  } catch (e) { return []; }
};

export const fetchOperationSummary = async (filters: any[] = []) => {
  try {
    return await ApiClient.post<any[]>('/operation/summary', { filters });
  } catch (e) { return []; }
};
...
export const generateMockData = (days: number = 30): EnergyRecord[] => {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    data.push({
      ...
      usageKWh: Math.random() * 20 + 10, cost: Math.random() * 5, 
      captiveCost: (Math.random() * 20 + 10) * 0.35, source: 'Grid'
    });
  }
  return data;
};
